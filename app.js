...
// ==================== 第10周真实样例 ====================
const WEEK_DATA = {
    indexUrl: 'data/behavior_logs/week_2026_w10_index.json',
...
async function applyPathNeedPathUserSelection(userId) {
    ...
    if (isFileProtocol()) {
        ...
        if (!ok) toast('行为日志索引加载失败，请检查 data/behavior_logs/ 是否已生成', 'err');
    }
    const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
...
async function loadUserPathById(userId, { silent } = {}) {
    ...
    if (location.protocol !== 'file:') {
        ...
        if (!loaded && !silent) {
            toast('行为日志索引未加载，请确认已运行 prepare_week_data.py', 'err');
        }
    }
    ...
    if (u?.log_file && u.log_rows > 0) {
        ...
        try {
            ...
            if (!res.ok) throw new Error('日志文件不存在，请重新运行 prepare_week_data.py');
