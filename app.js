/**
 * 橙子学 — 反馈分析助手（纯前端版）
 * 直接调用 OpenAI/DeepSeek/智谱 等兼容 API；分类等业务数据在 localStorage，默认智谱凭证见 config/ai_defaults.json
 */
...
function userMetaCellHtml(item) {
    return `<div class="batch-meta-stack">
        <div class="batch-meta-line">${membershipTypeCellHtml(item)}</div>
        <div class="batch-meta-line batch-meta-uid">${userIdCellHtml(item)}</div>
        <div class="batch-meta-line batch-meta-time">${esc(displayItemTime(item.time))}</div>
    </div>`;
}

function labelSourceCellHtml(item) {
    const l1 = item.labelL1 && item.labelL1 !== '—' ? esc(item.labelL1) : '—';
    const l2 = item.labelL2 && item.labelL2 !== '—' ? esc(item.labelL2) : '—';
    return `<div class="batch-label-stack">
        <div class="batch-label-line">
            <span class="batch-label-k">一级</span>
            <span class="batch-label-v">${l1}</span>
        </div>
        <div class="batch-label-line">
            <span class="batch-label-k">二级</span>
            <span class="batch-label-v">${l2}</span>
        </div>
    </div>`;
}
...
/** 从 config/ai_defaults.json 加载（同机部署共享，换设备访问同一服务即可用） */
let aiDefaultsFromServer = null;

async function loadAiDefaultsFromServer() {
    if (location.protocol === 'file:') return null;
    try {
        const res = await fetch('config/ai_defaults.json?_=' + Date.now());
        if (!res.ok) return null;
        const data = await res.json();
        if (!data || typeof data !== 'object') return null;
        aiDefaultsFromServer = {
            provider: data.provider || DEFAULT_AI_SETTINGS.provider,
            model: data.model || DEFAULT_AI_SETTINGS.model,
            apiKey: (data.apiKey || '').trim(),
            customBase: (data.customBase || '').trim()
        };
        return aiDefaultsFromServer;
    } catch (_) {
        return null;
    }
}
...
function getEffectiveAISettings() {
    let saved = {};
    try {
        saved = JSON.parse(localStorage.getItem('cz_settings') || '{}');
    } catch (_) { /* ignore */ }
    const server = aiDefaultsFromServer || {};
    return {
        provider: saved.provider || server.provider || DEFAULT_AI_SETTINGS.provider,
        model: saved.model || server.model || DEFAULT_AI_SETTINGS.model,
        apiKey: (saved.apiKey || '').trim() || (server.apiKey || '').trim() || '',
        customBase: (saved.customBase || '').trim() || (server.customBase || '').trim() || ''
    };
}

function persistAISettings(settings, { silent } = {}) {
    const s = {
        provider: settings.provider || DEFAULT_AI_SETTINGS.provider,
        model: settings.model || DEFAULT_AI_SETTINGS.model,
        apiKey: settings.apiKey || '',
        customBase: settings.customBase || ''
    };
...
function applyAISettingsToForm(settings) {
    ...
    const baseEl = document.getElementById('customBase');
    if (!providerEl) return;
    providerEl.value = settings.provider || DEFAULT_AI_SETTINGS.provider;
    if (modelEl) modelEl.value = settings.model || DEFAULT_AI_SETTINGS.model;
    if (keyEl) keyEl.value = settings.apiKey || '';
    if (baseEl) baseEl.value = settings.customBase || '';
    document.getElementById('customBaseRow').style.display =
        providerEl.value === 'custom' ? '' : 'none';
}
...
function getNeedPathUsersForPeriod(periodId) {
    return getClassifyItemsForPeriod(periodId).filter(
        i => resolveNeedPath(i) === true && i.userId && String(i.userId).trim() !== '—'
    );
}
...
function fillPathNeedPathUserSelect() {
    ...
    users.forEach(it => {
        ...
        const opt = document.createElement('option');
        opt.value = String(it.seq);
        opt.dataset.userId = uid;
        ...
        opt.dataset.message = it.message || '';
        opt.dataset.feedbackPreview = (it.message || '').slice(0, 80);
        sel.appendChild(opt);
    });
}
...
function getActivePathUserId() {
    const item = getActivePathClassifyItem();
    if (item?.userId) return String(item.userId).trim();
    if (getPathFillMode() === 'filter') {
        const opt = document.getElementById('pathNeedPathUser')?.selectedOptions?.[0];
        return opt?.dataset?.userId?.trim() || '';
    }
...
function resolvePathClassifyItem(periodId, seq, selectedOpt) {
    let item = getClassifyItemBySeq(periodId, seq);
    if (item || !selectedOpt?.dataset?.userId) return item;
    const uid = selectedOpt.dataset.userId.trim();
    const timeNorm = selectedOpt.dataset.feedbackTime || '';
    const candidates = getClassifyItemsForPeriod(periodId).filter(
        i => String(i.userId).trim() === uid
    );
    if (timeNorm && timeNorm !== '—') {
        item = candidates.find(i => displayItemTime(i.time) === timeNorm) || null;
    }
    return item || candidates[0] || null;
}

async function applyPathNeedPathUserSelection(seq) {
    const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
    const needSel = document.getElementById('pathNeedPathUser');
    const opt = needSel?.selectedOptions?.[0];
    const item = resolvePathClassifyItem(periodId, seq, opt);
    const uid = (item ? String(item.userId).trim() : '')
        || opt?.dataset?.userId?.trim()
        || '';
    if (!uid) return;
    ...
    applyPathUserFields(uid, u, item, opt);
    ...
    await loadUserPathById(uid, { silent: false, classifyItem: item });
}
...
function resolveFeedbackTime(batchItem, weekUser, period, options = {}) {
    ...
    const fromItem = tryNorm(batchItem?.time);
    if (fromItem) return fromItem;
    const fromWeekUser = tryNorm(weekUser?.feedback_time);
    if (fromWeekUser) return fromWeekUser;
    if (!allowDefaults) return '';
    return tryNorm(weekIndex?.default_feedback_time)
...
function updatePathTimeHint(batchItem, resolvedTime, weekUser) {
    ...
    if (!el) return;
    let src = '周期默认';
    if (batchItem?.time && batchItem.time !== '—') src = '反馈分类';
...
function getClassifyItemForUser(periodId, uid, feedbackTimeNorm) {
    const id = String(uid || '').trim();
    if (!id) return null;
    const candidates = getClassifyItemsForPeriod(periodId).filter(
        i => String(i.userId).trim() === id
    );
    if (feedbackTimeNorm && feedbackTimeNorm !== '—') {
        const byTime = candidates.find(i => displayItemTime(i.time) === feedbackTimeNorm);
...
function getClassifyItemBySeq(periodId, seq) {
    const s = String(seq ?? '').trim();
    if (!s) return null;
    return getClassifyItemsForPeriod(periodId).find(i => String(i.seq) === s)
        || (periodId === getCurrentPeriodId()
            ? batchItems.find(i => String(i.seq) === s)
            : null)
        || null;
}

function getActivePathClassifyItem() {
    const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
    if (getPathFillMode() === 'filter') {
        const sel = document.getElementById('pathNeedPathUser');
        const seq = sel?.value?.trim();
        if (!seq) return null;
        return getClassifyItemBySeq(periodId, seq);
    }
    const uid = document.getElementById('pathUid')?.value.trim();
    if (!uid) return null;
    return getClassifyItemForUser(periodId, uid);
}

function syncPathNeedPathUserSelect(periodId, batchItem) {
    const needSel = document.getElementById('pathNeedPathUser');
    if (!needSel || !batchItem) return;
    const seq = String(batchItem.seq ?? '').trim();
    if (seq && [...needSel.options].some(o => o.value === seq)) {
        needSel.value = seq;
        return;
    }
    const uid = String(batchItem.userId || '').trim();
    const timeNorm = displayItemTime(batchItem.time);
    const match = [...needSel.options].find(o =>
        o.dataset.userId === uid
        && (!timeNorm || timeNorm === '—' || o.dataset.feedbackTime === timeNorm)
    );
    if (match) needSel.value = match.value;
}
...
function applyPathUserFields(uid, u, batchItem, selectedOpt) {
    const fbEl = document.getElementById('pathFeedback');
    const timeEl = document.getElementById('pathTime');
    const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
    const period = getPeriodById(periodId);
    if (!selectedOpt && getPathFillMode() === 'filter') {
        selectedOpt = document.getElementById('pathNeedPathUser')?.selectedOptions?.[0];
    }
    const timeFromOpt = selectedOpt?.dataset?.feedbackTime || '';
    let item = batchItem
        || (selectedOpt?.value ? getClassifyItemBySeq(periodId, selectedOpt.value) : null)
        || getClassifyItemForUser(periodId, uid, timeFromOpt);
    if (!item && selectedOpt?.dataset?.userId) {
        item = {
            userId: selectedOpt.dataset.userId,
            time: selectedOpt.dataset.feedbackTime || '',
            message: selectedOpt.dataset.message || selectedOpt.dataset.feedbackPreview || ''
        };
    }
    const weekUser = u || weekIndex?.users?.find(x => String(x.user_id) === String(uid).trim());
    const allowDefaults = getPathFillMode() !== 'filter';

    if (fbEl) {
        const msg = (item?.message || '').trim()
            || (selectedOpt?.dataset?.message || '').trim()
            || (selectedOpt?.dataset?.feedbackPreview || '').trim();
        if (msg) fbEl.value = msg;
        else if (weekUser?.feedback_text) fbEl.value = weekUser.feedback_text;
        else fbEl.value = '';
    }

    let resolvedTime = resolveFeedbackTime(item, weekUser, period, { allowDefaults });
    if (!resolvedTime && timeFromOpt && timeFromOpt !== '—') {
        resolvedTime = normalizeFeedbackTime(timeFromOpt) || timeFromOpt;
    }
...
async function loadUserPathById(userId, { silent, classifyItem } = {}) {
    const uid = String(userId || '').trim();
    if (!uid) return;
    ...
    const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
    const batchItem = classifyItem || getActivePathClassifyItem() || getClassifyItemForUser(periodId, uid);
    syncPathNeedPathUserSelect(periodId, batchItem);
    const u = weekIndex?.users?.find(x => String(x.user_id) === uid);
...
async function loadWeekFeedback() {
    ...
    try {
        ...
        finalizeFeedbackImport(`已导入 ${batchItems.length} 条反馈，可直接批量分析`);
    } catch (e) {
        toast(e.message, 'err');
    } finally {
        hideLoading();
...
async function initSettings() {
    await loadAiDefaultsFromServer();
    ...
    applyAISettingsToForm(effective);

    // 本机尚无 Key 但服务端配置有 → 写入 localStorage，换浏览器同机也可用
    if (!(saved.apiKey || '').trim() && effective.apiKey) {
        persistAISettings(effective, { silent: true });
        const status = document.getElementById('settingsStatus');
        if (status) status.textContent = '✅ 已加载团队默认智谱配置';
    }
...
function getAPIConfig() {
    ...
    const apiKey = effective.apiKey || document.getElementById('apiKey')?.value || '';
    const customBase = effective.customBase || document.getElementById('customBase')?.value || '';
...
/** 关联模块已明确（非待定位） */
function isPathModuleExplicit(module) {
    const m = (module || '').trim();
    return !!(m && m !== '—' && m !== '待定位' && !/^待/.test(m));
}

function mentionsCantFind(blob) {
    return /找不到|搜不到|查不到|搜不出/i.test(blob || '');
}

function mentionsPathTraceIntent(blob) {
    return /查路径|用户路径|行为路径|搜索路径|怎么找|在哪找/i.test(blob || '');
}

/** 找不到/搜不到 + 模块已明确 → 不按路径问题处理，不需查路径 */
function cantFindWithExplicitModuleSkipsPath(module, blob) {
    return mentionsCantFind(blob) && isPathModuleExplicit(module) && !mentionsPathTraceIntent(blob);
}

/** 结合 AI 标注与建议/依据，推断是否需查路径（避免「建议定位资料」却未勾选） */
function inferNeedPathFromAnalysis(item) {
    const explicit = parseNeedPathFromResult(item.result);
    const suggest = parseLabeledField(item.result, '建议');
    const judgment = parseLabeledField(item.result, '判断依据');
    const userNeed = parseLabeledField(item.result, '用户需求');
    const status = parseLabeledField(item.result, '满足状态');
    const category = normalizeCategoryL2(
        item.categoryL2 || parseLabeledField(item.result, '二级分类')
    );
    const module = (item.relatedModule || parseLabeledField(item.result, '关联模块') || '').trim();
    const blob = [
        item.result,
        item.resultBody,
        suggest,
    ...
    const needsLocateResource = /定位.*(资源|资料|内容|试卷|题目|链接)|根据链接|链接.*定位|核实.*链接|核对.*(资源|资料|内容)|待定位|资料\s*ID/i.test(blob);
    const needsPathFromCantFind = mentionsCantFind(blob) && !isPathModuleExplicit(module);
    const needsPathTrace = mentionsPathTraceIntent(blob) || needsPathFromCantFind;
    const skipPathForCantFind = cantFindWithExplicitModuleSkipsPath(module, blob);
    const hasLink = /https?:\/\//i.test(item.message || '') || /https?:\/\//i.test(blob);

    if (explicit === true) return true;
    if (category === '查找资源' && !skipPathForCantFind) {
        if (!isPathModuleExplicit(module) || !mentionsCantFind(blob)) return true;
    }
    if (/❓|不确定/.test(status || '')) return true;
    if (module === '待定位' || /^待/.test(module)) return true;
    if (needsPathTrace) return true;
    if (needsLocateResource && (hasLink || needsPathTrace || category === '内容错误')) return true;

    if (explicit === false) {
        if (category === '内容错误' && needsLocateResource) return true;
        if (skipPathForCantFind) return false;
        return false;
    }
    if (needsLocateResource && hasLink) return true;
    if (skipPathForCantFind) return false;
    return explicit;
}

function resolveNeedPath(item) {
    if (!item || item.status !== 'done') return item?.needPath ?? null;
    if (item._needPathUserSet) return item.needPath;
    if (item.needPath === true) return true;
    const inferred = inferNeedPathFromAnalysis(item);
    if (inferred === true) return true;
    if (item.needPath === false) return false;
    return inferred;
}
...
function applyAnalysisResult(item) {
    ...
    item.needPath = inferNeedPathFromAnalysis(item);
...
function cellSlotPending(label = '待 AI 分析') {
    return `<div class="cell-slot-pending" aria-hidden="true"><span class="cell-slot-pending-icon"></span><span>${esc(label)}</span></div>`;
}

function batchRowClass(item) {
    const s = item.status || 'pending';
    if (s === 'done') return 'batch-row batch-row-done';
    if (s === 'running') return 'batch-row batch-row-running';
    if (s === 'error') return 'batch-row batch-row-error';
    return 'batch-row batch-row-pending';
}

function categoryL2CellHtml(item) {
    if (item.status === 'running') {
        return '<div class="cell-slot-running"><span class="cell-slot-running-dot"></span>分析中…</div>';
    }
    if (item.status === 'done') {
        ensureAiSnapshot(item);
    ...
    if (human && human !== '—') {
        return `<div class="batch-l2-human">
            <span class="analysis-tag analysis-tag-human">${esc(human)}</span>
            <span class="tag-src">人工标注</span>
        </div>`;
    }
    return cellSlotPending();
}
...
function resultBodyHtml(item) {
    ...
    if (item.status === 'running') {
        return '<div class="cell-result running cell-slot-running"><span class="cell-slot-running-dot"></span>分析中…</div>';
    }
    return cellSlotPending('等待批量分析');
}

function pathCheckHtml(item) {
    if (item.status === 'pending' || item.status === 'running') {
        return `<span class="path-check-tag path-check-disabled" title="完成 AI 分析后可勾选">
            <input type="checkbox" class="path-check-input" disabled>
            <span class="path-check-lbl">需查路径</span>
        </span>`;
    }
    const needPath = resolveNeedPath(item);
    const checked = needPath === true;
    const cls = needPath === true ? 'checked' : needPath === false ? 'unchecked' : '';
    const title = needPath === null
        ? '分析后自动填充，也可手动勾选'
...
function bindPathCheckboxes(tbody) {
    tbody.querySelectorAll('.path-check-input').forEach(cb => {
        cb.addEventListener('change', () => {
            ...
            item._needPathUserSet = true;
            if (item.status === 'done') saveClassifySnapshot(getCurrentPeriodId());
...
/** 上传/粘贴后自动解析并展示批量分析表格 */
function finalizeFeedbackImport(toastMsg) {
    showClassifyBelow();
    const pid = getCurrentPeriodId();
    ...
    if (countEl) countEl.textContent = `${batchItems.length} 条`;
...
function renderBatchTable() {
    ...
    tbody.innerHTML = batchItems.map(item => {
        ...
        return `<tr class="${batchRowClass(item)}" data-seq="${item.seq}" data-status="${esc(item.status || 'pending')}">
            <td class="col-num">${item.seq}</td>
            <td class="col-label-src">${labelSourceCellHtml(item)}</td>
            <td class="col-msg">
            ...
            <td class="col-meta">${userMetaCellHtml(item)}</td>
...
function updateBatchRow(seq, patch) {
    ...
    if (!tr) return;
    const labelSrcCell = tr.querySelector('.col-label-src');
    const metaCell = tr.querySelector('.col-meta');
    ...
    if (labelSrcCell) labelSrcCell.innerHTML = labelSourceCellHtml(item);
    if (metaCell) metaCell.innerHTML = userMetaCellHtml(item);
...
function initClassify() {
    ...
    const filePickerLabel = area.querySelector('label[for="fileInput"]');
    const openPicker = () => input.click();
    pickBtn?.addEventListener('click', e => { e.stopPropagation(); openPicker(); });
    filePickerLabel?.addEventListener('click', e => e.stopPropagation());
    area.addEventListener('click', e => {
        if (e.target.closest('label[for="fileInput"]') || e.target.closest('#pickFileBtn')) return;
        openPicker();
    });
    ...
    area.addEventListener('drop', async e => {
        ...
        await handleFeedbackFileUpload(file);
    });
    input.addEventListener('change', async () => {
    ...
    document.getElementById('editDocBtn')?.addEventListener('click', () => {
        ...
        if (!t) {
            toast('请先上传文件、加载测试数据或粘贴反馈内容', 'err');
            return;
        }
        finalizeFeedbackImport(`已刷新预览，共 ${batchItems.length} 条`);
    });
    hideClassifyBelow();

    let feedbackImportTimer;
    const scheduleFeedbackImport = () => {
        clearTimeout(feedbackImportTimer);
        feedbackImportTimer = setTimeout(() => {
            const t = document.getElementById('feedbackInput')?.value.trim();
            if (!t || isSpreadsheetGarbage(t)) return;
            if (batchItems.length || feedbackRowsCache?.length) {
                finalizeFeedbackImport();
            } else if (countFeedbackLines(t) > 0) {
                finalizeFeedbackImport();
            }
        }, 400);
    };
    ...
    document.getElementById('feedbackInput').addEventListener('input', () => {
        ...
        updateFeedbackCountDisplay();
    });

    document.getElementById('feedbackInput').addEventListener('paste', () => {
        scheduleFeedbackImport();
    });
...
function initPath() {
    ...
    document.getElementById('pathBtn').addEventListener('click', async () => {
        ...
        const periodId = document.getElementById('pathPeriod')?.value || getCurrentPeriodId();
        const batchItem = getActivePathClassifyItem() || getClassifyItemForUser(periodId, uid);
        if (!time) {
            const pathOpt = document.getElementById('pathNeedPathUser')?.selectedOptions?.[0];
            time = applyPathUserFields(
                uid,
                weekIndex?.users?.find(x => String(x.user_id) === uid),
                batchItem,
                pathOpt
            );
        }
...
function setFeedbackText(text) {
    ...
    syncBatchFromText();
    if (batchItems.length) finalizeFeedbackImport();
}
...
async function handleFeedbackFileUpload(file) {
    if (!file) return;
    showLoading('正在解析文件…');
    try {
        await readFile(file, 'feedbackInput');
    } catch (e) {
...
function updateFeedbackCountDisplay() {
    const countEl = document.getElementById('feedbackCount');
    const ta = document.getElementById('feedbackInput');
    ...
    countEl.textContent = `${batchItems.length || countFeedbackLines(t)} 条`;
}
...
async function readFile(file, targetId) {
    ...
    try {
        if (isFeedback && (isExcel || name.endsWith('.csv'))) {
            if (typeof XLSX === 'undefined') {
                throw new Error('表格解析库未加载完成，请稍候刷新页面后重试');
            }
            const rows = isExcel
            ...
            if (!rows?.length) {
                throw new Error('未解析到有效反馈，请确认表头含「有效信息」「用户ID」「反馈时间」等列');
            }
            ...
            updateFeedbackCountDisplay();
            finalizeFeedbackImport(`已导入 ${file.name}：${batchItems.length} 条，可直接批量分析`);
            return;
        }
